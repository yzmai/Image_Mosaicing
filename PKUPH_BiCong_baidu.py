...
# hurt_parent_folder = r'D:\Ynby\Doc\Demo\臂丛\多发伤&车祸伤/'
bicong_personData = pd.read_excel(bicong_parent_folder + '1.患者基本信息.xlsx', encoding="UTF-8", na_rep="", index=True)
# hurt_personData = pd.read_excel(hurt_parent_folder + '1、EMR.患者基本信息.xlsx', encoding="UTF-8", na_rep="", index=True)
# len(pd.merge(bicong_personData, hurt_personData, on='患者编号'))

bicong_BingShiData = pd.read_excel(bicong_parent_folder + '3.病史.xlsx', encoding="UTF-8", na_rep="", index=True)
# hurt_BingShiData = pd.read_excel(hurt_parent_folder + '3、EMR.病史(含查体).xlsx', encoding="UTF-8", na_rep="", index=True)
bicong_BingShiData = bicong_BingShiData.groupby(by='患者编号').apply(lambda t: t[t.就诊次数==t.就诊次数.max()])
# bicong_BingShiData.drop_duplicates(subset=['患者编号', '主诉', '现病史', '既往史', '家族史', '查体'], inplace=True)
...
for historyColname in historyColnames:
    ...
    WordFreqDf.to_csv(r'D:\Ynby\Doc\Demo/臂丛/臂丛'+ historyColname + '词频_baidu.csv', encoding="UTF-8", na_rep="", index=False)
    WordFreqDf.to_excel(r'D:\Ynby\Doc\Demo/臂丛/臂丛'+ historyColname + '词频_baidu.xlsx', encoding="UTF-8", na_rep="", index=False)
