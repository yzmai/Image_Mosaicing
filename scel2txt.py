...
if __name__ == '__main__':
    # scel所在文件夹路径
    in_path = r"D:\Ynby\Git\Image-Mosaicing-master\Dict\scel"
    # 输出词典所在文件夹路径
    out_path = r"D:\Ynby\Git\Image-Mosaicing-master\Dict\txt"
    ...
    out_path = r"D:\Ynby\Git\Image-Mosaicing-master\Dict\txt\GBK"
    for f in fin:
        try:
            for word in scel2txt(os.path.join(in_path, f)):
                strf = str(f)
                for eachstr in sepcharacter:
                    strf = strf.replace(eachstr, "")

                file_path = (os.path.join(out_path, strf.split('.')[0] + '.txt'))
                # 保存结果
