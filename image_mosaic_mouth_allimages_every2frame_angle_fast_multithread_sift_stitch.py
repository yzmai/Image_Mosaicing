...
class GenerateMosaic:
    ...
    def siftcal(self, thread_id):
        ...
        while len(self.img_id_stack) > 0:
            ...
            # Run RANSAC to remove outuliers
            ransac_obj = RANSAC()
            try:
    ...
    def substitch(self, thread_id, sub_img_name_list, key_offset, H_all, offset):
        print("\nThread : ", str(thread_id))
        for i, img_name in enumerate(sub_img_name_list):

            key = "H{}{}".format(i+key_offset, self.middle_id)
            H = H_all[key]
            ...
            self.canvas_img[thread_id] = fit_image_in_target_space(img_rgb, self.canvas_img[thread_id], self.mask[thread_id], np.linalg.inv(H),
                                                   offset=offset)  # the inp to fit_image_in_target_space
            # pts_in_img_2 = H * pts_in_canvas
            self.mask[thread_id][np.where(self.canvas_img[thread_id])[0:2]] = 0
    ...
    def stitch(self, H_all, result_fldr):
        ...
        for threadId in range(THREAD_NUM):
            ...
            sub_canvas_img_list[threadId] = threading.Thread(target=self.substitch, args=(threadId, sub_img_name_list, sub_img_length*threadId, H_all, offset, ))

        for threadId in range(THREAD_NUM):
            sub_canvas_img_list[threadId].start()
        for threadId in range(THREAD_NUM):
    ...
    def get_sub_blank_canvas(self, H_all, sub_img_name_list, key_offset):

        img_path = os.path.join(self.parent_folder, sub_img_name_list[0])
        ...
        for i in range(key_offset, key_offset+len(sub_img_name_list)):
...
if __name__ == "__main__":
    ...
    # img_name_list = img_name_list[int((len(img_name_list)/2)):]
