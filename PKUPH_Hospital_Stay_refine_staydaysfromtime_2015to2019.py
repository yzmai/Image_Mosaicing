...
newMergedSheet = pd.read_excel(os.path.join(parent_folder, '就诊基本信息2019-9至2020-8.xlsx'))

sheet_hosp = newMergedSheet
...
yearmonthlyMergedSheet = allMergedSheet.groupby('入院年月').agg({'患者编号':'count','住院天数_出入院时间':'mean'})
yearmonthlyMergedSheet.to_excel(outfolder+r'/入院年月统计_出入院时间_2019年8月后.xlsx', encoding="UTF-8", na_rep="", index=True)
yearmonthlyMergedSheet.to_csv(outfolder+r'/住院数据_入院年月统计_出入院时间_2019年8月后.csv', encoding="UTF-8", na_rep="", index=True)
