...
class RANSAC:
    ...
    def __init__(self, p=0.99, eps=0.8, n=6, delta=3):
        """
    ...
    def run_ransac(self, correspondence):
        ...
        distance_x = np.abs(correspondence[:, 2:3] - correspondence[:, 0:1])
        distance_y = np.abs(correspondence[:, 3:4] - correspondence[:, 1:2])
        distance = np.sqrt(np.power(correspondence[:, 2:3] - correspondence[:, 0:1], 2) + np.power(correspondence[:, 3:4] - correspondence[:, 1:2], 2))

        mean_distance_x = np.mean(distance_x)
        mean_distance_y = np.mean(distance_y)
        mean_distance = np.mean(distance)
        std_distance_x = np.std(distance_x)
        std_distance_y = np.std(distance_y)
        std_distance = np.std(distance)

        near_distance_x = distance_x < mean_distance_x + 3 * std_distance_x
        near_distance_y = distance_y < mean_distance_y + 3 * std_distance_y
        near_distance = distance < mean_distance + 3 * std_distance
        near_distance_x_index = list(np.where(near_distance_x == True)[0])
        near_distance_y_index = list(np.where(near_distance_y == True)[0])
        near_distance_index = list(np.where(near_distance == True)[0])

        near_distance_xy_list = list(set.intersection(set(near_distance_x_index), set(near_distance_y_index)))
        near_distance_xy_list = list(set.intersection(set(near_distance_xy_list), set(near_distance_index)))
        correspondence = correspondence[near_distance_xy_list]
