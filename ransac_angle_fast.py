...
class RANSAC:
    ...
    def run_ransac(self, correspondence):
        ...
        mean_distance_x = np.mean(np.abs(correspondence[:, 2:3] - correspondence[:, 0:1]))
        mean_distance_y = np.mean(np.abs(correspondence[:, 3:4] - correspondence[:, 1:2]))
        std_distance_x = np.std(np.abs(correspondence[:, 2:3] - correspondence[:, 0:1]))
        std_distance_y = np.std(np.abs(correspondence[:, 3:4] - correspondence[:, 1:2]))

        near_distance_x = np.abs(correspondence[:, 2:3] - correspondence[:, 0:1]) < mean_distance_x + 3 * std_distance_x
        near_distance_x_index = list(np.where(near_distance_x == True)[0])
        near_distance_y = np.abs(correspondence[:, 3:4] - correspondence[:, 1:2]) < mean_distance_y + 3 * std_distance_y
        near_distance_y_index = list(np.where(near_distance_y == True)[0])

        correspondence = correspondence[list(set.intersection(set(near_distance_x_index), set(near_distance_y_index)))]
        ...
        print("current_inliers_cnt:", current_inliers_cnt)
        print("len(current_sample_pts):", len(current_sample_pts))
        final_corr_points = np.concatenate((current_sample_pts, current_inliers), axis=0)
        print("len(final_corr_points):", len(final_corr_points))
